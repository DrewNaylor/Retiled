...
- A tile is no longer loaded if its .desktop file path is zero-length, which would cause problems before.
...
- Tiles are now sized by `TileSize` in the config file instead of raw height and width values.
  ...
  - You can switch to `TileSize` at any time by entering edit mode then leaving edit mode (long-pressing a tile then tapping it again), which in this case will force a save.
