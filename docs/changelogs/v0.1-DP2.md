...
- We're using `"".join(["/usr/share/applications/", DotDesktopFilePath])` instead of `"/usr/share/applications/" + DotDesktopFilePath` to prevent copying data where possible. Should reduce memory usage a little.
## Removed
