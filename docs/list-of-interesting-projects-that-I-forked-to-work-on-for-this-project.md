...
<br>As mentioned above, I'll need something for Retiled to use as a window manager/compositor under Wayland, so this project may help, though it might take some work to get it really usable as it mentions being a barely-function one. Doesn't have licenses or copyrights, so if I can't figure out what any of the related components use, hopefully it'll be ok to put it under the BSD 3-Clause License since the other similar project is under the same license. I'll be sure to put the copyright as the original developer, as well as myself for my changes. Originally forked from [malcolmstill/compositor.net](https://github.com/malcolmstill/compositor.net)
